abstract class OpenSSL::SSL::Socket
  ...
  def unbuffered_close
    ...
    begin
      loop do
        begin
          ret = LibSSL.ssl_shutdown(@ssl) # then we get back here...
          break if ret == 1 # success
          puts "non good ret #{ret}"
          raise OpenSSL::SSL::Error.new(@ssl, ret, "SSL_shutdown") if ret < 0 # get here for sure...
        rescue e : Errno
          case e.errno
          when 0
          ...
          when .want_read?, .want_write?
            # Ignore, shutdown did not complete yet I don't think we get here...
            puts "apparently was wan_read=#{e.error.want_read?} or write=#{e.error.want_write?}"
          else
            puts "re-raising"
            raise e # I think we get here...or actually maybe it's just in want_read :|
          end
        end
