...
# 3 days 100MB with this? 50MB.  1% cpu. 3 days 350MB?
spawn do
  loop do
    sleep 1.0
    GC.collect
...
get "/sync_web_server" do |env|
  ...
  if system("crystal build ./kemal_server.cr")
    puts "not doing stop, do that manually now..." 
    #Kemal.stop # should allow this process to die as well...
    "SUCCESS kemal has rebuilt and should NOT be bouncing..." # have to let it die so the bash script can set permissions :| this should be fast enough, right? I mean seriously...
  else
...
def create_new_and_redir(real_url, episode_number, episode_name, title, duration, env)
  ...
  if url_or_nil
    ...
    # Amazon.com: Watch Battlstar Galactica Season 1
...
settings = [
  ...
  {type: :everything, title: "Everything we have edited", urls: all_urls_done, message: "All movies and TV and youtubes we have edited!"},
